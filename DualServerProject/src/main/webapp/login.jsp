...
try {
	...
	case 1:
		try {
	rs = st.executeQuery(
			"select * from reg where name='" + name + "' AND pass='" + pass + "' AND role='owner'");
	if (rs.next()) {
	...
	int i = st.executeUpdate(
			"insert into reg(name, pass, email, dob, gen, phone, state, country,status,role) values ('" + name
					+ "','" + pass + "','" + Eamil + "','" + dob + "','" + Gender + "','" + phone + "','"
					+ State + "','" + Country + "','Pending','" + role + "')");
	if (i != 0) {
	...
	case 7:
		try {
	rs = st.executeQuery(
			"select * from reg where name='" + name + "' AND pass='" + pass + "' AND role='receiver'");
	if (rs.next()) {
		session.setAttribute("sssname", rs.getString("name"));
		session.setAttribute("sssemail", rs.getString("email"));
		session.setAttribute("sssstate", rs.getString("state"));
